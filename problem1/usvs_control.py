...
def change_state(Timeoftheworld, u_hat):
    for i in range(NumberofHunters):
        allhunters[i].change_position(u_hat[0], u_hat[1])# 改变每个状态
        allhunters[i].get_distance_and_angle(agent_invader.pos_x,agent_invader.pos_y)# 计算智能体与目标的距离和角度
    agent_invader.change_speed(0.2, 0.1)
    agent_invader.change_position()

def change_network(Timeoftheworld, optimal_V_hat):
    for i in range(NumberofHunters):
        # 得到智能体的邻居
        ...
        optimal_V_hat = np.linalg.inv(model_def.R_rewardweights) * allhunters[i].lambda_1.T * (-adp_drl_nn.zeta_1 * allhunters[i].z_1 - 1/2 * all_actor_1[i].forward(allhunters[i].z_1))
        ...
        allhunters[i].u_hat = -adp_drl_nn.zeta_2 * allhunters[i].z_2 - 1/2 * all_actor_2[i].finaloutputs
